  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      const shonux = {
        ...
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=94789088223:+947890888223\nEND:VCARD` } }
      };
...
END:VCARD`
        ...
        if (!q) {
            await socket.sendMessage(sender, { 
                ...
                buttons: [
                    { buttonId: `${config.PREFIX}menu`, buttonText: { d�isplayText: '📋 MENU' }, type: 1 }
                ]
            }, { quoted: botMention });
            return;
        }
...
async function setupCallRejection(socket, sessionNumber) {
    socket.ev.on('call', async (calls) => {
        try {
            ...
            const userConfig = await loadUse�rConfigFromMongo(sanitized) || {};
...
`, useBotName);
          ...
          try {
            ...
            try {
              if (String(useLogo).sta�rtsWith('http')) {
